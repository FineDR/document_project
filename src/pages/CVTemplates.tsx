...
// --- CONFIGURATION ---
// Set to true to enable the payment gateway modal. 
// Set to false to bypass payment and allow direct download.
...
const CVTemplates = () => {
  ...
  // UX: Lock body scroll when a modal is open
  useEffect(() => {
  ...
  const openPreview = (name: string) => {
    ...
    setSelectedTemplate(() => templateMap[name]);
  ...
  const handleUseTemplateClick = () => {
    if (paymentSuccess || !PAYMENT_ENABLED) {
      handleDownload(selectedTemplateName || "Basic");
    } else {
  ...
  const handleDownload = (templateName: string) => {
    let thunk;
    switch (templateName) {
      case "Basic": thunk = getBasicPDF(); break;
      case "Intermediate": thunk = getIntermediatePDF(); break;
      case "Advanced": thunk = getAdvancedPDF(); break;
      default: return;
    }
  ...
  return (
    <main className="w-full min-h-screen bg-background transition-colors duration-300 pb-20 pt-10 border-t border-subHeadingGray/10">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        {/* Templates Grid - Mobile: 1 col, Tablet: 2 col, Desktop: 3 col */}
            ...
            >
              ...
              >
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[210mm] origin-top transform scale-[0.3] md:scale-[0.4] shadow-md pointer-events-none opacity-90 group-hover:opacity-100 transition-opacity mt-4">
                   {/* Force light theme for thumbnail consistency */}
      ...
      <AnimatePresence>
        {isPreviewModalOpen && selectedTemplate && (
          ...
          >
            <motion.div
              // Mobile: h-full w-full (fullscreen), Desktop: Max height with rounded corners
            ...
            >
              ...
              {/* Mobile Close Button (Floating with background for visibility) */}
              <button
              ...
              {/* LEFT: Preview Canvas (Scrollable) */}
              {/* Mobile: flex-1 (takes all available space above controls) */}
              <div className="flex-1 bg-subHeadingGray/10 overflow-y-auto custom-scrollbar relative flex justify-center">
                <div className="py-8 px-2 md:px-4 min-h-full">
                   <div className="bg-white shadow-[0_0_30px_rgba(0,0,0,0.1)] w-full max-w-[210mm] min-h-[297mm]">
                     {/* Force light theme inside the A4 paper regardless of system theme */}
              ...
              {/* Mobile: Stacked at bottom, auto height. Desktop: Right side, fixed width */}
              <div className="w-full md:w-80 lg:w-96 bg-background border-t md:border-t-0 md:border-l border-subHeadingGray/20 flex flex-col z-20 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] md:shadow-none">
                ...
                {/* Sidebar Content */}
                {/* Hidden on very small mobile screens if space is tight, or scrollable */}
                ...
                <div className="p-4 md:p-6 border-t border-subHeadingGray/20 bg-background md:bg-subHeadingGray/5 pb-8 md:pb-6">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <Button
                      ...
                      className={`w-full py-3 md:py-4 text-base md:text-lg shadow-lg hover:shadow-xl transition-all rounded-xl active:scale-95 ${
                        !PAYMENT_ENABLED || paymentSuccess
                          ? "bg-green-600 hover:!bg-green-700 text-white" 
                          : "bg-redMain hover:!bg-red-700 text-white"
                      }`}
                    />
                  </div>
                </div>
